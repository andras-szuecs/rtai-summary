...
GD: $\eta^{t+1}\gets\eta^t - \gamma\cdot \nabla_\eta Loss(\eta^t)$
...
$\mathcal{N}\!(x;\mu,\sigma^2) \!=\! {\frac{1}{\sqrt{(2\pi)^d |\Sigma|}}exp(-\frac{1}{2}(x-\mu)^T\Sigma^{-1} (x-\mu))}$
...
\textbf{Hölder's:} $||x \cdot y||_1 \leq ||x||_p \cdot ||y||_q$, if $\frac{1}{p} + \frac{1}{q} = 1$
...
$\Phi(z)=1-\Phi(-z), \Phi^{-1}(1-p)= -\Phi^{-1}(p)$
