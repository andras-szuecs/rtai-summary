...
Data Extraction (exact training samples), MI.
\subsection{Federated Learning}

\textbf{FedSGD:}
Entities do training step on minibatch \({x^k, y^k}\) from private data \(\mathcal{D}_k\)
...
Honest but curious server: does not manipulate sent weights.
...
\subsection{Membership Inference}:Attacker trains shadow models on the same data distribution, some with entry $x$, some without.
If logits are given, then attacker trains a classifier to distinguish between the two cases.
If not, then do the same with robustness scores eg (LiRa).
\textbf{LLM} MI: shadow models too expensive to train$\rightarrow$weaker (more computable) membership signals, eg $score_{MIA}(x;\theta)=\mathbb{P}(x|\theta)$ (assuming access to loss/logits of $\theta$).
loss/\textbf{p}er\textbf{pl}exity based:$MIA(x;\theta)=1[\mathcal{L}(x;\theta)<\gamma]$, $\frac{PPL(\theta,x)}{PPL(\theta,\hat{x}_i)}<\gamma$(PPL ratio), $\frac{PPL(\theta,x)}{zLib(x)}<\gamma$(compr. ratio)
Min K-Prob: $MIA(x;\theta)=1[MIN_k \mathbb{P}(x)>\epsilon]$
\subsection{Dataset Inference}: Validation and Public dataset($B_{val},B_{pub}$)$\rightarrow$ feature extraction + regression $\rightarrow$ Aggregation ($\mu_{M(B_{val})}, \mu_{M(B_{pub})}$)$\rightarrow$ Hypothesis testing ($H_0$: $B_{pub}$ not used. $p$ val $\overset{\cdot}{=}\mathbb{P}(\text{data}|H_0)$)
\subsection{Memorisation}{\sethlcolor{red}\hl{TODO}}
\subsection{Attribute Inference}{\sethlcolor{red}\hl{TODO}}

\subsection{Agents}: fundamental issue: no separation between data and instruction

\subsection{Differential Privacy}

%MI protection is \mhl{\(\mathbb{P}({M}(\mathcal{D}) \in S) \approx \mathbb{P}({M}(\textcolor{blue}{\mathcal{D}}) \in S)\)}
\extratext{Membership Inference protection is \mhl{\(\mathbb{P}({M}(\mathcal{D}) \in S) \approx \mathbb{P}({M}(\textcolor{blue}{\mathcal{D}}) \in S)\)}}

\({M}\) is \(\varepsilon\)-DP if \(\forall (a, a')\in \text{Neigh}\)
and \(\forall \text{(attack) }S\subseteq \mathbb{B}\):
\mhl{\(\mathbb{P}({M}(a) \in S) \leq e^\varepsilon {\mathbb{P}({M}(a') \in S)} 
%\iff \mathbb{P}({M}(a) = b) \leq e^\varepsilon {\mathbb{P}({M}(a') = b)}\)} for $b\in\mathbb{B}$.
\Leftrightarrow \mathbb{P}({M}(a) = b) \leq e^\varepsilon {\mathbb{P}({M}(a') = b)}\)} for $b\in\mathbb{B}$.
